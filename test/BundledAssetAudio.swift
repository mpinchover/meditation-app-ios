...
enum BundledAssetAudio {
    ...
    /// Groups by the first path component; top-level files use `otherTitle` (default `"Other"`). Folders sorted A–Z, then Other last.
    static func groupedSections(relativePaths: [String], otherTitle: String = "Other") -> [(title: String, files: [String])] {
        var buckets: [String: [String]] = [:]
        for p in relativePaths {
            let parts = p.split(separator: "/").map(String.init)
            let key = parts.count <= 1 ? otherTitle : parts[0]
            buckets[key, default: []].append(p)
        }
        let titles = buckets.keys.sorted { a, b in
            if a == otherTitle { return false }
            if b == otherTitle { return true }
            return a.localizedCaseInsensitiveCompare(b) == .orderedAscending
        }
        return titles.compactMap { title in
            guard let files = buckets[title] else { return nil }
            let sorted = files.sorted { $0.localizedCaseInsensitiveCompare($1) == .orderedAscending }
            return (title, sorted)
        }
    }
